...
(function network(window) {
    ...
    if (request.status === 200) {
      window.net = JSON.parse(request.responseText);
    }
}(this));
...
function fingerprinter(visitorId) {
    let finalData = {jscd, ware, net};

    var fingerprint = {};
    fingerprint[visitorId] = finalData;

    postwith('post.aspx', JSON.stringify(fingerprint));
